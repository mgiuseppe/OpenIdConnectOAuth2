...
using MG_IdentityProvider.Quickstart.UserRegistration;
...
namespace IdentityServer4.Quickstart.UI
{
    ...
    public class ExternalController : Controller
    {
        ...
        public async Task<IActionResult> Callback()
        {
            ...
            if (user == null)
            {
                // custom workflow for user registration
                var returnUrlAfterRegistration = Url.Action(
                    nameof(Callback), 
                    new { returnUrl });

                var continueWithUrl = Url.Action(
                    nameof(UserRegistrationController.RegisterUser), 
                    "UserRegistration",
                    new { returnUrl = returnUrlAfterRegistration, provider, providerUserId });
        ...
        private (User user, string provider, string providerUserId, IEnumerable<Claim> claims) FindUserFromExternalProvider(AuthenticateResult result)
        {
            ...
            // find external user
            var user = _mgUserRepository.GetUserByProvider(provider, providerUserId);
